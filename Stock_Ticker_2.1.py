...
def nyse_ticker():
    name = entry.get()
    web_site = requests.get(f'https://www.google.com/finance/quote/{name}:NYSE')
    soup = bs4.BeautifulSoup(web_site.text, 'lxml')
    stock = soup.find('div', class_='YMlKec fxKbKc').text
    label_1 = Label(window, text=f"Current of {name} Price is {stock}")
    label_1.pack()
...
# Create a buttons
button_1 = Button(window, text="NASDAQ", padx=50, pady=5, relief=tk.RAISED, borderwidth=5, command=nasdaq_ticker)
button_1.pack()

button_2 = Button(window, text="NYSE", padx=50, pady=5, relief=tk.RAISED, borderwidth=5, command=nyse_ticker)
button_2.pack()
